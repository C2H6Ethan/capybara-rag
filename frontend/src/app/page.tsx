  ...
  const send = useCallback(async (text: string) => {
    ...
    try {
        ...
        while ((idx = buf.indexOf("\n\n")) >= 0) {
          ...
          if (event.type === "chunk" && event.content) {
            if (firstToken) {
              firstToken = false;
              setThinking(false);
              setMessages((m) => [...m, { id: replyId, role: "capy", text: "", streaming: true }]);
            }
            setMessages((m) =>
              m.map((x) =>
            ...
            setMessages((m) =>
              m.map((x) =>
                x.id === replyId && x.role === "capy" ? { ...x, sources: event.sources } : x
              )
            );
            ...
            if (firstToken) {
              firstToken = false;
              setThinking(false);
              setMessages((m) => [...m, { id: replyId, role: "capy", text: event.content!, streaming: false }]);
            }
          }
        }
      }
      ...
      if (firstToken) {
        setMessages((m) => [
          ...m,
          { id: replyId, role: "capy", text: "Something went wrong — make sure the backend is running.", streaming: false },
        ]);
      }
      console.error(err);
    }
    ...
    setMessages((m) =>
      m.map((x) => (x.id === replyId && x.role === "capy" ? { ...x, streaming: false } : x))
    );
  }, []);
