...
Next.js Frontend (localhost:3000)
  CapyGPT chat UI — SSE consumer, streaming caret, thinking state
     │
     │  POST /api/ask  (Next.js rewrites → localhost:8000)
     ▼
...
│   │   ├── embedder.py        # Batch embeds chunks → stores in pgvector
...
This loads `data/chunks/chunks_size200_overlap50.json`, embeds all 105 chunks locally using `all-MiniLM-L6-v2`, and stores them in pgvector. Takes ~15 seconds on CPU.
...
Wikipedia / specialty sites
   ...
   data/raw/           17 articles scraped manually
        ...
        │  (keep only paragraphs with ≥1 capybara mention per 1000 words)
        ▼
